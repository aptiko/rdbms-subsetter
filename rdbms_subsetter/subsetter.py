...
Usage::
    ...
    rdbms-subsetter <source> <destination> <fraction>

"source" and "destination" are SQLAlchemy connection strings. "fraction" is the fraction
of rows to use.
...
consider using the ``-l`` flag, which reduces row counts by a logarithmic formula.  If
``f`` is the fraction specified, and ``-l`` is set, and the original table has ``n``
rows, then each new table's row target will be::
        ...
        try:
            if self.db.engine.driver in (
                ...
                raise NotImplementedError(
                    "No approximation known for driver %s" % self.db.engine.driver
                )
    ...
    if self.n_rows:
        while True:
            ...
            if self.n_rows > 1000:
                ...
                ).where(self.random_row_func() < fraction)
...
def _table_matches_any_pattern(schema, table, patterns):
    ...
    return any(
        fnmatch.fnmatch(qual_name, each) or fnmatch.fnmatch(table, each)
...
class Db:
    def __init__(self, sqla_conn, args, schemas=[None]):
        ...
        for schema in self.schemas:
            ...
            for tbl in meta.sorted_tables:
                if args.tables and not _table_matches_any_pattern(
                    ...
                    continue
                if _table_matches_any_pattern(
                    tbl.schema, tbl.name, self.args.exclude_tables
                ):
                    continue
                tbl.db = self
                ...
                if not tbl.pk:
                    tbl.pk = [
                        d["name"]
                        for d in self.inspector.get_columns(tbl.name, schema=tbl.schema)
                    ]
        ...
        for ((tbl_schema, tbl_name), tbl) in self.tables.items():
            ...
            for fk in tbl.fks + constraints:
                ...
                fk[
                    "constrained_table"
                ] = tbl_name  # TODO: check against constrained_table
                self.tables[
            ...
            if _table_matches_any_pattern(tbl.schema, tbl.name, self.args.full_tables):
                ...
                if tbl.n_rows:
                    if self.args.logarithmic:
                        target.n_rows_desired = (
                            int(
                                math.pow(
                                    10, math.log10(tbl.n_rows) * self.args.fraction
                                )
                            )
                            or 1
                        )
                        ...
                        target.n_rows_desired = (
                            int(tbl.n_rows * self.args.fraction) or 1
                        )
                else:
    ...
    def confirm(self):
        ...
        for (tbl_schema, tbl_name) in sorted(self.tables, key=lambda t: t[1]):
            ...
            message.append(
                "Create %d rows from %d in %s.%s"
                % (tbl.target.n_rows_desired, tbl.n_rows, tbl_schema or "", tbl_name)
            )
        print("\n".join(sorted(message)))
        if self.args.yes:
            return True
    ...
    def create_row_in(self, source_row, target_db, target, prioritized=False):
        ...
        for child_fk in target.child_fks:
            child = self.tables[
                (child_fk["constrained_schema"], child_fk["constrained_table"])
            ]
            slct = sa.sql.select([child])
            for (child_col, this_col) in zip(
                child_fk["constrained_columns"], child_fk["referred_columns"]
            ):
    ...
    def pending(self):
        return functools.reduce(
            lambda count, table: count + len(table.pending), self.tables.values(), 0
        )
    ...
    def create_subset_in(self, target_db):
        ...
        for (tbl_name, pks) in self.args.force_rows.items():
            ...
            for pk in pks:
                ...
                if source_row:
                    self.create_row_in(
                        source_row, target_db, source.target, prioritized=True
                    )
                else:
                    logging.warn(
                        "requested %s:%s not found in source db,"
                        "could not create" % (source.name, pk)
                    )

        while True:
            targets = sorted(
                target_db.tables.values(), key=lambda t: t.completeness_score()
            )
            try:
                ...
                break
            logging.debug(
                "total n_rows in target: %d"
                % sum((t.n_rows for t in target_db.tables.values()))
            )
            logging.debug(
                "target tables with 0 n_rows: %s"
                % ", ".join(t.name for t in target_db.tables.values() if not t.n_rows)
            )
            logging.info(
                "lowest completeness score (in %s) at %f"
...
argparser.add_argument(
    "source", help="SQLAlchemy connection string for data origin", type=str
)
argparser.add_argument(
    "dest", help="SQLAlchemy connection string for data destination", type=str
)
argparser.add_argument(
    "fraction", help="Proportion of rows to create in dest (0.0 to 1.0)", type=fraction
)
argparser.add_argument(
    "-l",
    "--logarithmic",
...
argparser.add_argument(
    "--loglevel", type=loglevel, help="log level (%s)" % all_loglevels, default="INFO"
)
argparser.add_argument(
    "-f",
    "--force",
...
argparser.add_argument(
    "--config", help="Path to configuration .json file", type=argparse.FileType("r")
)
argparser.add_argument(
    "--table",
    "-t",
...
argparser.add_argument(
    ...
    dest="exclude_tables",
    help=(
        "Tables to exclude. When both -t and -T are given, the behavior is to "
        "include just the tables that match at least one -t switch but no -T switches."
    ),
    type=str,
    action="append",
    default=[],
...
argparser.add_argument(
    "-y", "--yes", help="Proceed without stopping for confirmation", action="store_true"
)
...
def hashable(raw):
    ...
    result = tuple(hashable(itm) if isinstance(itm, list) else itm for itm in raw)
    return result
